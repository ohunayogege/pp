...
@section('content')
    ...
    @if (!empty($home->html))
    {!! replaceBaseUrl(convertUtf8($home->html)) !!}
    @else
      <div class="well py-5 text-center bg-light" style="direction: ltr;">
          <h3>Use Page Builder to decorate content of <strong>selected theme & {{$currentLang->name}} language</strong></h3><br>
          <a href="{{route('admin.homeTheme', ['language' => $currentLang->code])}}" target="_blank">Click Here</a>
      </div>
    @endif
