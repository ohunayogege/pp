                                ...
                                @auth
                                    @if ($activeSub->count() > 0 && empty($activeSub->first()->next_package_id))
                                        @if ($activeSub->first()->current_package_id == $package->id)
                                            <a href="{{route('front.packageorder.index',$package->id)}}" class="lawyer_btn">{{__('Extend')}}</a>
                                        @else
                                            <a href="{{route('front.packageorder.index',$package->id)}}" class="lawyer_btn">{{__('Change')}}</a>
                                        @endif
                                    @elseif ($activeSub->count() == 0)
                                        <a href="{{route('front.packageorder.index',$package->id)}}" class="lawyer_btn">{{__('Purchase')}}</a>
                                    @endif
                                @endauth
                                @guest
                                    <a href="{{route('front.packageorder.index',$package->id)}}" class="lawyer_btn">{{__('Purchase')}}</a>
