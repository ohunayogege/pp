...
<link
   rel="stylesheet"
   href="{{ asset('assets/front/css/magnific-popup.css') }}"
   >
<link
   rel="stylesheet"
      ...
      <div class="row">
         <div class="col-lg-6">
            <div class="courses-img-box">
               <img
                  data-src="{{ asset('assets/front/img/courses/' . $course_details->course_image) }}"
                  class="lazy img-fluid"
                  alt=""
                  >
               <div class="video-box">
                  <a
                     href="{{ $course_details->video_link }}"
                     class="video-popup"
         ...
         <div class="col-lg-6">
            <div class="course-content">
               ...
               <p class="price">
                  ...
                  @else
                  <span class="off-price">{{ $bse->base_currency_symbol_position == 'left' ? $bse->base_currency_symbol : '' }} {{ $course_details->current_price }} {{ $bse->base_currency_symbol_position == 'right' ? $bse->base_currency_symbol : '' }}</span>
                  @if (!empty($course_details->previous_price))
                  <span class="main-price">{{ $bse->base_currency_symbol_position == 'left' ? $bse->base_currency_symbol : '' }} {{ $course_details->previous_price }} {{ $bse->base_currency_symbol_position == 'right' ? $bse->base_currency_symbol : '' }}</span>
                  @endif
                  @endif
               </p>
               <p>{{ $course_details->summary }}</p>
               <ul class="info">
                  <li>{{ __('Category') . ':' }}<span><a href="{{route('courses', ['categroy_id' => $course_details->course_category_id])}}">{{ $course_details->courseCategory->name }}</a></span></li>
               </ul>
               <form
                  method="POST"
                  id="paymentGatewayForm" enctype="multipart/form-data"
                  >
                  ...
                  <div class="d-flex flex-row">
                     ...
                     @auth
                        @php
                            $pcourse = Auth::user()->courseOrder()->where('course_id', $course_details->id)->where('payment_status', 'completed');
                        @endphp
                         @if(Auth::check() && $pcourse->count() > 0)
                            <strong class="text-danger mb-3 mr-4">{{__('Already Purchased')}}</strong><br>
                            <a class="text-primary" href="{{route('user.course.lessons', $pcourse->first()->id)}}">{{__('Go to this Course')}}</a>
                         @else
                            <div class="form_group {{ $rtl == 1 ? 'ml-3' : 'mr-3' }} {{ $course_details->current_price == null ? 'd-none' : '' }}">
                                <select name="gateway"
                                id="paymentType"
                            ...
                            <div class="form-group button mb-20">
                                ...
                                type="submit"
                                id="enrollBtn"
                                class="main-btn {{ $course_details->current_price == null ? 'd-none' : '' }}"
                                >{{ __('Enroll Now') }}</button>
                            </div>
                            <div class="form-group button mb-20">
                                <button
                                type="submit"
                     ...
                     <div class="col-md-6 mb-3">
                        ...
                        <div class="field-input mb-2">
                           <input
                              ...
                              id="cvc-number-id"
                              class="card-elements"
                              placeholder="{{ __('Enter CVC Number') }}"
                              name="cvcNumber"
                              oninput="checkCVCNum(this.value);"
                              >
                        </div>
               ...
               <div class="tab-content">
                        ...
                        <div
                           ...
                           <div class="card">
                              <a
                                 ...
                                 >{{ $module->name }}<span class="toggle_btn"></span>
                              </a>
                              <div
                                 ...
                                 <div class="card-body">
                                    ...
                                    <ul>
                                       ...
                                       <li><a><i class="fas fa-play"></i> {{ $lesson->name }} <span class="duration">{{ $lesson->duration }}</span></a></li>
                                       @endforeach
                                    </ul>
                                 </div>
                              </div>
                  ...
                  <div
                     ...
                     <div class="instructor-wrap">
                        <div class="thumb">
                           <img
                              data-src="{{ asset('assets/front/img/instructors/' . $course_details->instructor_image) }}"
                        ...
                        <div class="content">
                           ...
                           <ul class="social-link">
                               ...
                               <li><a href="{{$course_details->instructor_facebook}}" target="_blank"><i class="fab fa-facebook-f"></i></a></li>
                               @endif
                               @if (!empty($course_details->instructor_twitter))
                               <li><a href="{{$course_details->instructor_twitter}}" target="_blank"><i class="fab fa-twitter"></i></a></li>
                               @endif
                               @if (!empty($course_details->instructor_instagram))
                               <li><a href="{{$course_details->instructor_instagram}}" target="_blank"><i class="fab fa-instagram"></i></a></li>
                               @endif
                               @if (!empty($course_details->instructor_linkedin))
                               <li><a href="{{$course_details->instructor_linkedin}}" target="_blank"><i class="fab fa-linkedin-in"></i></a></li>
                               @endif
                           </ul>
                        </div>
                     </div>
                     ...
                     <div class="shop-review-area">
                        ...
                        <div class="mb-5">
                           ...
                           <div class="review_user">
                              @if (strpos($review->reviewByUser->photo, 'facebook') !== false || strpos($review->reviewByUser->photo, 'google'))
                              <img class="lazy" data-src="{{$review->reviewByUser->photo ? $review->reviewByUser->photo : asset('assets/front/img/user/profile.jpg')}}" alt="user image">
                              @else
                              <img class="lazy" data-src="{{$review->reviewByUser->photo ? asset('assets/front/img/user/'.$review->reviewByUser->photo) : ''}}" alt="user image">
                              @endif
                              <div class="rate">
                                 <div class="rating" style="width: {{$review->rating * 20}}%"></div>
                              </div>
                              @if ($rtl == 1)
                              <span><span>{{ $review->created_at->format('M d, Y') }}</span> {{ '- ' . $review->reviewByUser->fname . ' ' . $review->reviewByUser->lname }}</span>
                              @else
                              <span><span>{{ $review->reviewByUser->fname . ' ' . $review->reviewByUser->lname }}</span> {{ '- ' . $review->created_at->format('M d, Y') }}</span>
                              @endif
                              <p>{{ $review->comment }}</p>
                           </div>
                           @endforeach
                        ...
                        <div class="review_form pt-4">
                           <form action="{{ route('course.review') }}" method="POST">
                              ...
                              <div class="form_group">
                                 ...
                                 <textarea
                                    class="form_control mt-2"
                                    name="comment"
                                    placeholder="{{ __('Write something about this course') }}"
                                    ></textarea>
                              </div>
