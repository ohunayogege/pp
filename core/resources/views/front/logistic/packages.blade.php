                                ...
                                @auth
                                    @if ($activeSub->count() > 0 && empty($activeSub->first()->next_package_id))
                                        @if ($activeSub->first()->current_package_id == $package->id)
                                            <a href="{{route('front.packageorder.index',$package->id)}}" class="logistics_btn">{{__('Extend')}}</a>
                                        @else
                                            <a href="{{route('front.packageorder.index',$package->id)}}" class="logistics_btn">{{__('Change')}}</a>
                                        @endif
                                    @elseif ($activeSub->count() == 0)
                                        <a href="{{route('front.packageorder.index',$package->id)}}" class="logistics_btn">{{__('Purchase')}}</a>
                                    @endif
                                @endauth
                                @guest
                                    <a href="{{route('front.packageorder.index',$package->id)}}" class="logistics_btn">{{__('Purchase')}}</a>
                            ...
                            @else
                                @if ($package->order_status != 0)
                                    ...
                                    <div class="pricing_button">
                                        <a href="{{ $link }}" @if($package->order_status == 2) target="_blank" @endif class="logistics_btn">{{__('Place Order')}}</a>
                                    </div>
                                @endif
                            @endif
                        </div>
